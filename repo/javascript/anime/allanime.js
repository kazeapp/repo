...
class DefaultExtension extends KProvider {
  ...
  async getPopular(page) {
    ...
    const resList = JSON.parse(
      await this.request(encodedGql)
    ).data.queryPopular.recommendations.filter((e) => e.anyCard !== null);
    ...
    for (const anime of resList) {
      ...
      const link = `/bangumi/${anime.anyCard._id}/${anime.anyCard.name
        .replace(/[^a-zA-Z0-9]/g, "-")
        .replace(/-{2,}/g, "-")
        .toLowerCase()}`;
  ...
  async search(query, page, filters) {
    ...
    for (const anime of resList) {
      ...
      const link = `/bangumi/${anime._id}/${anime.name
        .replace(/[^a-zA-Z0-9]/g, "-")
        .replace(/-{2,}/g, "-")
        .toLowerCase()}`;
      list.push({ name, imageUrl, link });
  ...
  async getVideoList(url) {
    ...
    for (const video of videoJson.data.episode.sourceUrls) {
      ...
      let quality = "";
      if (
        videoUrl.includes("/apivtwo/") &&
        altHosterSelection.some((element) => "player" === element)
      ) {
        ...
        ["vidstreaming", "https://gogo", "playgo1.cc", "playtaku"].some(
          (element) => videoUrl.includes(element)
        ) &&
        altHosterSelection.some((element) => "vidstreaming" === element)
      ) {
        ...
        ["dood", "d0"].some((element) => videoUrl.includes(element)) &&
        altHosterSelection.some((element) => "dood" === element)
      ) {
        ...
        altHosterSelection.some((element) => "okru" === element)
      ) {
        ...
        videoUrl.includes("mp4upload.com") &&
        altHosterSelection.some((element) => "mp4upload" === element)
      ) {
        ...
        videoUrl.includes("streamlare.com") &&
        altHosterSelection.some((element) => "streamlare" === element)
      ) {
...
class AllAnimeExtractor {
  ...
  async videoFromUrl(url, name) {
    const videoList = [];
    const endPointResponse = JSON.parse(
      (await new Client().get(`${this.baseUrl}/getVersion`, this.headers)).body
    );
    const endPoint = endPointResponse.episodeIframeHead;

    const resp = await new Client().get(
      endPoint + url.replace("/clock?", "/clock.json?"),
      this.headers
    );
        ...
        if (link.mp4) {
          ...
          if (resp.statusCode === 200) {
            ...
            if (masterPlaylist.includes("#EXT-X-MEDIA:TYPE=AUDIO")) {
              const audioInfo = masterPlaylist
                .substringAfter("#EXT-X-MEDIA:TYPE=AUDIO")
                .substringBefore("\n");
              ...
              masterPlaylist
                ...
                .forEach((it) => {
                  ...
                  if (it.includes("AVERAGE-BANDWIDTH")) {
                    bandwidth = ` ${this.bytesIntoHumanReadable(
                      it
                        .substringAfter("AVERAGE-BANDWIDTH=")
                        .substringBefore(",")
                    )}`;
                  }
                  const quality = `${it
                    ...
                    .substringAfter("x")
                    .substringBefore(",")}p${bandwidth} (${name} - ${
                    link.resolutionStr
                  })`;
                  ...
                  if (!videoUrl.startsWith("http")) {
                    videoUrl =
                      resp.request.url.substringBeforeLast("/") +
                      `/${videoUrl}`;
                  }
                  const headers = {
                    Host: videoUrl.match(
            ...
            if (stream.format === "adaptive_dash") {
              videoList.push({
                ...
                quality: `Original (AC - Dash${
                  stream.hardsub_lang.length === 0
                    ? ""
                    : ` - Hardsub: ${stream.hardsub_lang}`
                })`,
          ...
          const audios =
            link.rawUrls && link.rawUrls.audios
              ? link.rawUrls.audios.map((it) => {
                  return {
                    file: it.url,
              ...
              : [];
          const videos =
            link.rawUrls && link.rawUrls.vids
              ? link.rawUrls.vids.map((it) => {
                  if (!audios) {
                    return {
                      url: it.url,
                      quality: `${name} - ${
                        it.height
                      } ${this.bytesIntoHumanReadable(it.bandwidth)}`,
                      originalUrl: it.url,
                      subtitles,
                    };
                  } else {
